...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess.old2' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess.old2' ) ) {
		$writable = true;
	} else {
		$writable = false;
		$existing_rules  = array_filter( extract_from_markers( $home_path . '.htaccess.old2', 'WordPress' ) );
...
if ( ! is_multisite() ) {
	if ( $iis7_permalinks ) {
		...
		if ( $permalink_structure && ! $usingpi && ! $writable && $update_required ) {
			_e('You should update your .htaccess.old2 now.');
		} else {
			_e('Permalink structure updated.');
		}
	}
...
<?php elseif ( ! $is_nginx ) :
	if ( $permalink_structure && ! $usingpi && ! $writable && $update_required ) : ?>
<p><?php _e('If your <code>.htaccess.old2</code> file were <a href="https://codex.wordpress.org/Changing_File_Permissions">writable</a>, we could do this automatically, but it isn&#8217;t so these are the mod_rewrite rules you should have in your <code>.htaccess.old2</code> file. Click in the field and press <kbd>CTRL + a</kbd> to select all.') ?></p>
<form action="options-permalink.php" method="post">
<?php wp_nonce_field('update-permalink') ?>
